...
//    a.foldRight(Some(Nil): Option[List[A]])((opt,  optList) => optList flatMap (l => opt     map (_ :: l)))

  def traverse[A, B](a: List[A])(f: A => Option[B]): Option[List[B]] =
    a.foldRight(Some(Nil): Option[List[B]])((elem, optList) => optList flatMap (l => f(elem) map (_ :: l)))
}